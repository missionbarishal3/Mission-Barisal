...
export class SsotManager {
    ...
    constructor(
        /** Config root — where .zombiecoder/ lives (OS home: $HOME/.zombiecoder/) */
        private readonly configRoot: string,
        private readonly log: (message: string) => void,
        /** Project root — active workspace to scan for SSOT (may differ from config root) */
    ...
    public get ssotPath(): string {
        return path.join(this.configRoot, '.zombiecoder', 'SSOT.md');
    }
...
`;
    ...
    public regenerate(): string {
        const targetDir = path.join(this.configRoot, '.zombiecoder');
        const targetPath = path.join(targetDir, 'SSOT.md');
        // Scan the active project workspace, not the config root
        const scanDir = this._projectRoot ?? this.configRoot;
        ...
        try {
            ...
            const projectInfo = this.scanProject(scanDir);
            const content = this.generateSSOT(scanDir, projectInfo);
