...
export const TOKEN_CONSTANTS = {
  ...
  /** Hard cap on max_tokens — even if the model supports more, we never
   *  request more than this to prevent runaway 100K+ char responses. */
  MAX_OUTPUT_HARD_CAP: 16384,
...
export function calculateSafeMaxOutputTokens(params: SafeOutputTokensParams): number {
  ...
  const safeMaxOutputTokens = Math.min(
    ...
    TOKEN_CONSTANTS.MAX_OUTPUT_HARD_CAP,
